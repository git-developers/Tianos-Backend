...
var OneToManyCoordMap = Class.extend({

	grid: null, // reference to the OneToMany
...
var OneToMany = fc.OneToMany = RowRenderer.extend({
	...
	coordMap: null, // a OneToManyCoordMap that converts pixel values to datetimes
	elsByFill: null, // a hash of jQuery element sets used for rendering each fill. Keyed by fill name.

	documentDragStartProxy: null, // binds the OneToMany's scope to documentDragStart (in DayOneToMany.events)

	// derived from options
	colHeadFormat: null, // TODO: move to another class. not applicable to all OneToManys
	...
	constructor: function() {
		...
		this.coordMap = new OneToManyCoordMap(this);
		this.elsByFill = {};
	...
	// TODO: move to another class. not applicable to all OneToManys
	...
	// Tells the grid about what period of time to display. OneToMany will subsequently compute dates for cell system.
	...
	bindHandlers: function() {
		...
		// attach event-element-related handlers. in OneToMany.events
	...
	/* Event Helper
	------------------------------------------------------------------------------------------------------------------*/
	// TODO: should probably move this to OneToMany.events, like we did event dragging / resizing
    ...
    /* Event-rendering and event-interaction methods for the abstract OneToMany class
----------------------------------------------------------------------------------------------------------------------*/
...
OneToMany.mixin({
	...
	// Generic enough to work with any type of OneToMany.
	segDragMousedown: function(seg, ev) {
		...
		var dragListener = new DragListener(view.coordMap, {
			...
			cellOver: function(cell, isOrig) {
				var origCell = seg.cell || dragListener.origCell; // starting cell could be forced (DayOneToMany.limit)
...
var DayOneToMany = OneToMany.extend({
	...
	render: function(isRigid) {
		...
		OneToMany.prototype.render.call(this); // call the super-method
	},


	destroy: function() {
		this.destroySegPopover();
		OneToMany.prototype.destroy.call(this); // call the super-method
	},
	...
	// specifically for whole-day rows, whereas a 'bg' might also be used for other purposes (TimeOneToMany bg for example).
	...
	// Overrides OneToMany's method for when row coordinates are computed
	computeRowCoords: function() {
		var rowCoords = OneToMany.prototype.computeRowCoords.call(this); // call the super-method
	...
	------------------------------------------------------------------------------------------------------------------*/
	// TODO: move to DayOneToMany.event, similar to what we did with OneToMany's drag methods


	// Renders a visual indication of an event or external element being dragged.
	// The dropLocation's end can be null. seg can be null. See OneToMany::renderDrag for more info.
	renderDrag: function(dropLocation, seg) {
		var opacity;
    ...
    /* Event-rendering methods for the DayOneToMany class
----------------------------------------------------------------------------------------------------------------------*/
...
DayOneToMany.mixin({
	...
	destroyEvents: function() {
		...
		OneToMany.prototype.destroyEvents.apply(this, arguments); // calls the super-method
	},


	// Retrieves all rendered segment objects currently rendered on the grid
	getEventSegs: function() {
		return OneToMany.prototype.getEventSegs.call(this) // get the segments from the super-method
	...
	renderBgSegs: function(segs) {
		...
		return OneToMany.prototype.renderBgSegs.call(this, allDaySegs); // call the super-method
	},
...
function compareDaySegCols(a, b) {
    ...
    /* Methods relate to limiting the number events for a given day on a DayOneToMany
----------------------------------------------------------------------------------------------------------------------*/
...
var TimeOneToMany = OneToMany.extend({
	...
	constructor: function() {
		OneToMany.apply(this, arguments); // call the super-constructor
		this.processOptions();
	},
	...
	// dropLocation's end might be null, as well as `seg`. See OneToMany::renderDrag for more info.
    ...
    /* Event-rendering methods for the TimeOneToMany class
----------------------------------------------------------------------------------------------------------------------*/
...
----------------------------------------------------------------------------------------------------------------------*/
// It is a manager for a DayOneToMany subcomponent, which does most of the heavy lifting.
// It is responsible for managing width/height.

var BasicView = fcViews.basic = View.extend({

	dayOneToMany: null, // the main subcomponent that does most of the heavy lifting
	...
	initialize: function() {
		this.dayOneToMany = new DayOneToMany(this);
		this.coordMap = this.dayOneToMany.coordMap; // the view's date-to-cell mapping is identical to the subcomponent's
	},
	...
	setRange: function(range) {
		...
		this.dayOneToMany.breakOnWeeks = /year|month|week/.test(this.intervalUnit); // do before setRange
		this.dayOneToMany.setRange(range);
	},
	...
	render: function() {
		...
		this.dayNumbersVisible = this.dayOneToMany.rowCnt > 1; // TODO: make grid responsible
		this.weekNumbersVisible = this.opt('weekNumbers');
		this.dayOneToMany.numbersVisible = this.dayNumbersVisible || this.weekNumbersVisible;
		...
		this.dayOneToMany.coordMap.containerEl = this.scrollerEl; // constrain clicks/etc to the dimensions of the scroller

		this.dayOneToMany.el = this.el.find('.fc-day-grid');
		this.dayOneToMany.render(this.hasRigidRows());
	},


	// Make subcomponents ready for cleanup
	destroy: function() {
		this.dayOneToMany.destroy();
				...
				'<thead>' +
					'<tr>' +
						'<td class="' + this.widgetHeaderClass + '">' +
							this.dayOneToMany.headHtml() + // render the day-of-week headers
						'</td>' +
					'</tr>' +
				'</thead>' +
				'<tbody>' +
	...
	// Queried by the DayOneToMany subcomponent when generating rows. Ordering depends on isRTL.
	...
	// Queried by the DayOneToMany subcomponent. Ordering depends on isRTL.
	numberIntroHtml: function(row) {
		if (this.weekNumbersVisible) {
			return '' +
				'<td class="fc-week-number" ' + this.weekNumberStyleAttr() + '>' +
					'<span>' + // needed for matchCellWidths
						this.calendar.calculateWeekNumber(this.dayOneToMany.getCell(row, 0).start) +
					'</span>' +
				'</td>';
		}
	},
	...
	// Generates the HTML that goes before every other type of row generated by DayOneToMany. Ordering depends on isRTL.
	...
	// Generates the HTML for the <td>s of the "number" row in the DayOneToMany's content skeleton.
	...
	numberCellHtml: function(cell) {
		...
		classes = this.dayOneToMany.getDayClasses(date);
	...
	setHeight: function(totalHeight, isAuto) {
		...
		this.dayOneToMany.destroySegPopover(); // kill the "more" popover if displayed

		// is the event limit a constant level number?
		if (eventLimit && typeof eventLimit === 'number') {
			this.dayOneToMany.limitRows(eventLimit); // limit the levels first so the height can redistribute after
		}

		scrollerHeight = this.computeScrollerHeight(totalHeight);
		this.setOneToManyHeight(scrollerHeight, isAuto);

		// is the event limit dynamically calculated?
		if (eventLimit && typeof eventLimit !== 'number') {
			this.dayOneToMany.limitRows(eventLimit); // limit the levels after the grid's row heights have been set
		}
	...
	// Sets the height of just the DayOneToMany component in this view
	setOneToManyHeight: function(height, isAuto) {
		if (isAuto) {
			undistributeHeight(this.dayOneToMany.rowEls); // let the rows be their natural height with no expanding
		}
		else {
			distributeHeight(this.dayOneToMany.rowEls, height, true); // true = compensate for height-hogging rows
		}
	},
	...
	renderEvents: function(events) {
		this.dayOneToMany.renderEvents(events);
	...
	getEventSegs: function() {
		return this.dayOneToMany.getEventSegs();
	},
	...
	destroyEvents: function() {
		...
		this.dayOneToMany.destroyEvents();
	...
	// A returned value of `true` signals that a mock "helper" event has been rendered.
	renderDrag: function(dropLocation, seg) {
		return this.dayOneToMany.renderDrag(dropLocation, seg);
	},


	destroyDrag: function() {
		this.dayOneToMany.destroyDrag();
	},
	...
	renderSelection: function(range) {
		this.dayOneToMany.renderSelection(range);
	},


	// Unrenders a visual indications of a selection
	destroySelection: function() {
		this.dayOneToMany.destroySelection();
	}
...
var MonthView = fcViews.month = BasicView.extend({
	...
	setOneToManyHeight: function(height, isAuto) {
		...
		distributeHeight(this.dayOneToMany.rowEls, height, !isAuto); // if auto, don't compensate for height-hogging rows
	},
...
----------------------------------------------------------------------------------------------------------------------*/
// Is a manager for the TimeOneToMany subcomponent and possibly the DayOneToMany subcomponent (if allDaySlot is on).
...
fcViews.agenda = View.extend({ // AgendaView
	...
	timeOneToMany: null, // the main time-grid subcomponent of this view
	dayOneToMany: null, // the "all-day" subcomponent. if all-day is turned off, this will be null
	...
	initialize: function() {
		this.timeOneToMany = new TimeOneToMany(this);

		if (this.opt('allDaySlot')) { // should we display the "all-day" area?
			this.dayOneToMany = new DayOneToMany(this); // the all-day subcomponent of this view

			// the coordinate grid will be a combination of both subcomponents' grids
			this.coordMap = new ComboCoordMap([
				this.dayOneToMany.coordMap,
				this.timeOneToMany.coordMap
			]);
		}
		else {
			this.coordMap = this.timeOneToMany.coordMap;
		}
	},
	...
	setRange: function(range) {
		...
		this.timeOneToMany.setRange(range);
		if (this.dayOneToMany) {
			this.dayOneToMany.setRange(range);
		}
	},
	...
	render: function() {
		...
		this.timeOneToMany.coordMap.containerEl = this.scrollerEl; // don't accept clicks/etc outside of this

		this.timeOneToMany.el = this.el.find('.fc-time-grid');
		this.timeOneToMany.render();

		// the <hr> that sometimes displays under the time-grid
		this.bottomRuleEl = $('<hr class="' + this.widgetHeaderClass + '"/>')
			.appendTo(this.timeOneToMany.el); // inject it into the time-grid

		if (this.dayOneToMany) {
			this.dayOneToMany.el = this.el.find('.fc-day-grid');
			this.dayOneToMany.render();

			// have the day-grid extend it's coordinate area over the <hr> dividing the two grids
			this.dayOneToMany.bottomCoordPadding = this.dayOneToMany.el.next('hr').outerHeight();
		}
		...
		return '' +
			'<table>' +
				'<thead>' +
					'<tr>' +
						'<td class="' + this.widgetHeaderClass + '">' +
							this.timeOneToMany.headHtml() + // render the day-of-week headers
						'</td>' +
					'</tr>' +
				'</thead>' +
				'<tbody>' +
	...
	// Generates the HTML that will go before the day-of week header cells.
	// Queried by the TimeOneToMany subcomponent when generating rows. Ordering depends on isRTL.
	headIntroHtml: function() {
		var date;
		var weekNumber;
		var weekTitle;
		...
		if (this.opt('weekNumbers')) {
			date = this.timeOneToMany.getCell(0).start;
	...
	// Generates the HTML that goes before the bg of the TimeOneToMany slot area. Long vertical column.
	...
	// Queried by the TimeOneToMany and DayOneToMany subcomponents when generating rows. Ordering depends on isRTL.
	introHtml: function() {
	...
	setHeight: function(totalHeight, isAuto) {
		...
		if (this.dayOneToMany) {
			...
			if (eventLimit) {
				this.dayOneToMany.limitRows(eventLimit);
			}
		}
	...
	initializeScroll: function() {
		...
		var top = this.timeOneToMany.computeTimeTop(scrollTime);
	...
	renderEvents: function(events) {
		...
		timedSegs = this.timeOneToMany.renderEvents(timedEvents);
		if (this.dayOneToMany) {
			daySegs = this.dayOneToMany.renderEvents(dayEvents);
		}
	...
	// Retrieves all segment objects that are rendered in the view
	getEventSegs: function() {
		return this.timeOneToMany.getEventSegs().concat(
			this.dayOneToMany ? this.dayOneToMany.getEventSegs() : []
		);
	},
	...
	destroyEvents: function() {
		...
		this.timeOneToMany.destroyEvents();
		if (this.dayOneToMany) {
			this.dayOneToMany.destroyEvents();
		}
	...
	renderDrag: function(dropLocation, seg) {
		if (dropLocation.start.hasTime()) {
			return this.timeOneToMany.renderDrag(dropLocation, seg);
		}
		else if (this.dayOneToMany) {
			return this.dayOneToMany.renderDrag(dropLocation, seg);
		}
	},
	...
	destroyDrag: function() {
		this.timeOneToMany.destroyDrag();
		if (this.dayOneToMany) {
			this.dayOneToMany.destroyDrag();
		}
	},
	...
	renderSelection: function(range) {
		if (range.start.hasTime() || range.end.hasTime()) {
			this.timeOneToMany.renderSelection(range);
		}
		else if (this.dayOneToMany) {
			this.dayOneToMany.renderSelection(range);
		}
	...
	// Unrenders a visual indications of a selection
	destroySelection: function() {
		this.timeOneToMany.destroySelection();
		if (this.dayOneToMany) {
			this.dayOneToMany.destroySelection();
		}
	}
